...
CI 会运行 `doctor`、`validate`、`plan`、Hermes profile plan、`audit`、`harness`、`hermes`、`memory`、`triggers`、`evals`、`memory-plugin` dry-run、Python CLI 测试，以及 DesignDNA CLI 测试，确保结构、兼容、安全、自动触发、验证性项目、harness readiness、self-improving agent readiness、agent memory readiness 和基础工具链可用。
